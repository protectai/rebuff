...
export default async function handler(
    ...
    const { action, inputText } = req.body;
    ...
    } else if (action === "detectLeakage") {
        const { outputText, canaryWord } = req.body;

        const isLeaked = outputText.includes(canaryWord);

        res.status(200).json({
            leakageDetected: isLeaked,
        });
    } else {
